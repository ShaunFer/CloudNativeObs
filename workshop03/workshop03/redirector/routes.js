...
router.get("/resolve/:code", async (req, res) => {
  ...
  httpRequestInFlightUpdownCounter.add(1, {
    [ATTR_URL_PATH] : '/code/:code'
  })
...
async function recordVisit(req, shortCode) {
  ...
  let resolved = true
    ...
    restApiDurationHistogram.record(Date.now() - startTime, {
      get_resolved: resolve
    })
  }
}
